...
// RMSNorm applied independently on each head of a [B, S, N*H] tensor
template <typename T>
inline at::Tensor head_rms_norm(
    at::Tensor t_in,
    at::Tensor t_wt,
    long N,
    long H,
  ...
  auto t_out = llama_rms_norm<T>(t_in.view({-1, N, H}), t_wt, eps);
  return t_out.view(sizes);
}
...
struct __attribute__((visibility("hidden"))) Qwen3DecoderLayer : LLMBlock {
 public:
  at::Tensor t_Wq, t_Wk, t_Wv, t_Wp;
  at::Tensor t_Wg, t_Wu, t_Wd;
  at::Tensor t_Gi, t_Gpa;
  at::Tensor t_Bq, t_Bk, t_Bv;
  at::Tensor t_Gq, t_Gk; // q_norm / k_norm gamma, per head_dim
  at::Tensor t_EP; // embed_positions
  at::Tensor t_Wq_1, t_Wk_1, t_Wv_1, t_Wp_1;
  at::Tensor t_Wg_1, t_Wu_1, t_Wd_1;
  bool first_token_remapped = false;
  ...
  Qwen3DecoderLayer(
    ...
    t_Bq = params[i++]; // q_proj bias (may be empty)
    t_Bk = params[i++]; // k_proj bias (may be empty)
    t_Bv = params[i++]; // v_proj bias (may be empty)

    t_Gq = params[i++]; // q_norm gamma
    t_Gk = params[i++]; // k_norm gamma
    ...
    if (t_Wq.is_quantized()) {
      ...
      if (USE_MXFP4 || USE_QINT8 || USE_QINT2) {
        if (t_Wq.dtype() == at::kBFloat16) {
          remap_for_first_token<bfloat16>();
        } else {
          remap_for_first_token<float>();
  ...
  virtual std::vector<at::Tensor> forward(
      ...
      c10::intrusive_ptr<TppCache> t_cache,
      bool use_cache) override {
    return this->template forward_common<Qwen3DecoderLayer>(
        t_inp, t_cache, use_cache);
  }
  ...
  std::vector<at::Tensor> _forward(
    ...
    t_QL = head_rms_norm<T>(t_QL, t_Gq, Nq, H, eps);
    t_KL = head_rms_norm<T>(t_KL, t_Gk, Nkv, H, eps);
    apply_rotary_pos_emb_llama<T>(t_QL, t_EP, t_pid, Nq, H);
    apply_rotary_pos_emb_llama<T>(t_KL, t_EP, t_pid, Nkv, H);
...
REGISTER_SUBMODULE(_fused_llm_infer, m) {
  ...
  py::class_<Qwen3DecoderLayer>(m, "Qwen3DecoderLayer", py::module_local())
      .def(py::init<std::vector<at::Tensor>, long, double, long, long, long>())
      .def("forward", &Qwen3DecoderLayer::forward);
}
...
TORCH_LIBRARY(tpp_llm, m) {
  ...
  m.class_<Qwen2DecoderLayer>("Qwen2DecoderLayer")
      .def(torch::
               init<std::vector<at::Tensor>, long, double, long, long, long>())
      .def("forward", &Qwen2DecoderLayer::forward);
  m.class_<Qwen3DecoderLayer>("Qwen3DecoderLayer")
      .def(torch::
               init<std::vector<at::Tensor>, long, double, long, long, long>())
      .def("forward", &Qwen3DecoderLayer::forward);
}
