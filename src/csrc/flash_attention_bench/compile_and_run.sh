...
if [ "$llm" == "llama-7b" -o "$llm" == "llama4" -o "$llm" == "llama-3.1-8B" ]; then
  ...
  embed_size=8192; nheads=64; head_size=128; bias_flag=0; nbbias_flag=0
elif [ "$llm" == "llama-3.2-3B" ]; then
  embed_size=3072; nheads=24; head_size=128; bias_flag=0; nbbias_flag=0
elif [ "$llm" == "llama-3.2-1B" ]; then
  embed_size=2048; nheads=32; head_size=64; bias_flag=0; nbbias_flag=0
elif [ "$llm" == "gpt2" ]; then
  embed_size=768; nheads=12; head_size=64; bias_flag=0; nbbias_flag=0
  ...
  embed_size=256; nheads=8; head_size=32; bias_flag=1; nbbias_flag=1
elif [ "$llm" == "alphafold2-h16" ]; then
  embed_size=64; nheads=4; head_size=16; bias_flag=1; nbbias_flag=1
elif [ "$llm" == "alphafold2-h8" ]; then
  embed_size=64; nheads=8; head_size=8; bias_flag=1; nbbias_flag=1
else
