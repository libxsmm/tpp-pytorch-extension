...
auto q_data_a = GetVLAPtr<T>(q_data, {S_t, N_t * H_t});
...
auto qkv_brgemm_tpp = SCOPEITGEMM(
    (BrgemmTPP<
        T,
        T>(QKVO_BLOCKSIZE, QKVO_BLOCKSIZE, QKVO_BLOCKSIZE, QKVO_BLOCKSIZE, QKVO_BLOCKSIZE*QKVO_BLOCKSIZE, N_t*H_t, QKVO_BLOCKSIZE, QKVO_BLOCKSIZE, 0.0, 0, 1)));

auto scale_tpp = SCOPEIT((ScaleTPP<T, T>(QKVO_BLOCKSIZE * QKVO_BLOCKSIZE)), EW_SCL);
    ...
    for (int i = 0; i < B_t; i++) {
      for (int j = 0; j < S_t; j += QKVO_BLOCKSIZE) {
        for (int k = 0; k < (N_t*H_t); k += QKVO_BLOCKSIZE) {
          ...
          qkv_brgemm_tpp(&q_data_a[i][j][0], &query_w_a[k/QKVO_BLOCKSIZE][0][0][0], &tmp[0], (N_t*H_t)/QKVO_BLOCKSIZE);
...
#pragma omp parallel for collapse(3)
    for (int i = 0; i < B_t; i++) {
      for (int j = 0; j < S_t; j += QKVO_BLOCKSIZE) {
        for (int k = 0; k < (N_t*H_t); k += QKVO_BLOCKSIZE) {
          LIBXSMM_ALIGNED(T tmp[QKVO_BLOCKSIZE * QKVO_BLOCKSIZE], 64);
          qkv_brgemm_tpp(&m_data_a[i][j][0], &key_w_a[k/QKVO_BLOCKSIZE][0][0][0], &tmp[0], (N_t*H_t)/QKVO_BLOCKSIZE);
...
#pragma omp parallel for collapse(3)
    for (int i = 0; i < B_t; i++) {
      for (int j = 0; j < S_t; j += QKVO_BLOCKSIZE) {
        for (int k = 0; k < (N_t*H_t); k += QKVO_BLOCKSIZE) {
          LIBXSMM_ALIGNED(T tmp[QKVO_BLOCKSIZE * QKVO_BLOCKSIZE], 64);
          qkv_brgemm_tpp(&m_data_a[i][j][0], &value_w_a[k/QKVO_BLOCKSIZE][0][0][0], &tmp[0], (N_t*H_t)/QKVO_BLOCKSIZE);
...
if (S_t < 2048) {
  auto a_brgemm_tpp = SCOPEITGEMM((BrgemmTPP<T, T>(
      QKVO_BLOCKSIZE, QKVO_BLOCKSIZE, H_t, 0, 0, N_t * H_t, S_t, S_t, 0.0, 0, 1)));
  ...
  auto a_add_sfmask_tpp =
      SCOPEIT(AddBiasTPP<float>(QKVO_BLOCKSIZE, S_t - Sp_t, S_t), BIAS);
  auto a_softmax_tpp =
...
#pragma omp parallel for collapse(3)
      for (int i = 0; i < B_t; i++) {
        for (int n = 0; n < N_t; n++) {
          for (int j1 = 0; j1 < S_t; j1 += QKVO_BLOCKSIZE) {
            ...
            LIBXSMM_ALIGNED(T tmp_logits[QKVO_BLOCKSIZE][S_t], 64);
  ...
  auto a_addbias_online_edge_tpp =
      SCOPEIT(AddBiasTPP<float>(QKVO_BLOCKSIZE, lastBlockSize, lastBlockSize), BIAS);
...
auto g_sigmoid_tpp =
    SCOPEIT(SiLUFwdTPP<T>(QKVO_BLOCKSIZE, QKVO_BLOCKSIZE), EW_MUL);
    ...
    for (int i = 0; i < B_t; i++) {
      for (int j = 0; j < S_t; j += QKVO_BLOCKSIZE) {
        ...
        for (int k=0; k < (N_t*H_t); k += QKVO_BLOCKSIZE) {
          ...
          qkv_brgemm_tpp(&q_data_a[i][j][0], &gating_w_a[k/QKVO_BLOCKSIZE][0][0][0], &tmp[0], (N_t*H_t)/QKVO_BLOCKSIZE);
        ...
        for (int k=0; k < (N_t*H_t); k += QKVO_BLOCKSIZE) {
          LIBXSMM_ALIGNED(T tmp[QKVO_BLOCKSIZE * QKVO_BLOCKSIZE], 64);
          qkv_brgemm_tpp(
              &tmp_gate_values[0], &output_w_a[k/QKVO_BLOCKSIZE][0][0][0], &tmp[0], (N_t*H_t)/QKVO_BLOCKSIZE);
          if (bias_flag)
      ...
      for (int i = 0; i < B_t; i++) {
        for (int j = 0; j < S_t; j += QKVO_BLOCKSIZE) {
          for (int k=0; k < (N_t*H_t); k += QKVO_BLOCKSIZE) {
            ...
            qkv_brgemm_tpp(
                &weighted_avg_a[i][j][0], &output_w_a[k/QKVO_BLOCKSIZE][0][0][0], &tmp[0], (N_t*H_t)/QKVO_BLOCKSIZE);
            if (bias_flag)
              out_addbias_tpp(&output_b_a[k][0], &tmp[0]);
            copy_tpp(&tmp[0], &output_a[i][j][k]);
          }
