...
auto q_data_a = GetVLAPtr<T>(q_data, {S_t/QKVO_BLOCKSIZE, N_t, QKVO_BLOCKSIZE, H_t});
...
auto q_a = GetVLAPtr<T>(q, {S_t/QKVO_BLOCKSIZE, N_t, QKVO_BLOCKSIZE * H_t});

T* kn = new (std::align_val_t(64)) T[B_t * S_t * N_t * H_t];
auto kn_a = GetVLAPtr<T>(kn, {S_t/QKVO_BLOCKSIZE, N_t, QKVO_BLOCKSIZE * H_t});

T* k = new (std::align_val_t(64)) T[B_t * S_t * N_t * H_t];
auto k_a = GetVLAPtr<T>(k, {S_t/QKVO_BLOCKSIZE, N_t, QKVO_BLOCKSIZE * H_t});

T* vn = new (std::align_val_t(64)) T[B_t *S_t * N_t *H_t];
auto vn_a = GetVLAPtr<T>(vn, {S_t/QKVO_BLOCKSIZE, N_t, QKVO_BLOCKSIZE * H_t});

T* v = new (std::align_val_t(64)) T[B_t *S_t * N_t *H_t];
auto v_a = GetVLAPtr<T>(v, {S_t/QKVO_BLOCKSIZE, N_t, QKVO_BLOCKSIZE * H_t});

T* weighted_avg = new (std::align_val_t(64)) T[B_t * S_t * N_t * H_t];
auto weighted_avg_a = GetVLAPtr<T>(weighted_avg, {S_t/QKVO_BLOCKSIZE, N_t, QKVO_BLOCKSIZE * H_t});

auto output_a = GetVLAPtr<T>(output, {S_t/QKVO_BLOCKSIZE, N_t, QKVO_BLOCKSIZE, H_t});

auto qgo_brgemm_tpp = SCOPEITGEMM(
    (BrgemmTPP<
        T,
        float>(QKVO_BLOCKSIZE, H_t, H_t, QKVO_BLOCKSIZE*H_t, H_t*H_t, H_t, H_t, H_t, 0.0, 0, 1, b_vnni)));
auto q_convert_tpp =
...
auto kv_brgemm_tpp = SCOPEITGEMM(
    (BrgemmTPP<
        T,
        T>(QKVO_BLOCKSIZE, H_t, H_t, QKVO_BLOCKSIZE*H_t, H_t*H_t, H_t, H_t, H_t, 0.0, 0, 1, b_vnni)));

auto scale_tpp = SCOPEIT((ScaleTPP<float, float>(QKVO_BLOCKSIZE * H_t)), EW_SCL);
    ...
    for (int i = 0; i < B_t; i++) {
      for (int j = 0; j < (S_t/QKVO_BLOCKSIZE); j++) {
        for (int k = 0; k < N_t; k++) {
          ...
          if (b_vnni)
            qgo_brgemm_tpp(&q_data_a[i][j][0][0][0], &qkv_w_vnni_a[k][0][0][0], &tmp[0], N_t);
          else
            qgo_brgemm_tpp(&q_data_a[i][j][0][0][0], &query_w_a[k][0][0][0], &tmp[0], N_t);
          scale_tpp(&tmp[0], &tmp[0], alpha);
          q_convert_tpp(&tmp[0], &q_a[i][j][k][0]);
        }
      }
    }
  }
    ...
    for (int i = 0; i < B_t; i++) {
      for (int j = 0; j < (S_t/QKVO_BLOCKSIZE); j++) {
        for (int k = 0; k < N_t; k++) {
          ...
          if (b_vnni)
            kv_brgemm_tpp(&m_data_a[i][j][0][0][0], &qkv_w_vnni_a[k][0][0][0], &tmp[0], N_t);
          else
            kv_brgemm_tpp(&m_data_a[i][j][0][0][0], &key_w_a[k][0][0][0], &tmp[0], N_t);
          copy_tpp(&tmp[0], &kn_a[i][j][k][0]);
          // k_trans_tpp(&tmp[0], &k_a[i][j][k][0]);
        }
      }
    }
  }
...
#pragma omp parallel for collapse(3)
  for (int i = 0; i < B_t; i++) {
    for (int j = 0; j < (S_t/QKVO_BLOCKSIZE); j++) {
      for (int k = 0; k < N_t; k++) {
        k_trans_tpp(&kn_a[i][j][k][0], &k_a[i][j][k][0]);
      }
    }
  }
      ...
      for (int j = 0; j < (S_t/QKVO_BLOCKSIZE); j++) {
        for (int k = 0; k < N_t; k++) {
          ...
          if(b_vnni) {
            ...
            kv_brgemm_tpp(&m_data_a[i][j][0][0][0], &value_w_a[k][0][0][0], &tmp[0], N_t);
          }
          copy_tpp(&tmp[0], &vn_a[i][j][k][0]);
        }
      }
    }
  }
...
#pragma omp parallel for collapse(3)
  for (int i = 0; i < B_t; i++) {
    for (int j = 0; j < (S_t/QKVO_BLOCKSIZE); j++) {
      for (int k = 0; k < N_t; k++) {
        if(b_vnni) {
          v_vnni_trans_tpp(&vn_a[i][j][k][0], &v_a[i][j][k][0]);
        } else {
          copy_tpp(&vn_a[i][j][k][0], &v_a[i][j][k][0]);
        }
      }
    }
  }
...
auto output_w_vnni_a = GetVLAPtr<T>(output_w_vnni, {N_t, H_t, H_t});
      ...
      for (int j = 0; j < (S_t/QKVO_BLOCKSIZE); j++) {
        ...
        for (int k=0; k < N_t; k++) {
          ...
          if (b_vnni)
            ...
            qgo_brgemm_tpp(&q_data_a[i][j][0][0][0], &gating_w_a[k][0][0][0], &tmp[0], N_t);
        ...
        for (int k=0; k < N_t; k++) {
          ...
          if (b_vnni)
            qgo_brgemm_tpp(
                &tmp_bf16[0][0], &output_w_vnni_a[k][0][0][0], &tmp[0], N_t);
          else
            qgo_brgemm_tpp(
                &tmp_bf16[0][0], &output_w_a[k][0][0][0], &tmp[0], N_t);
          if (bias_flag)
      ...
      for (int i = 0; i < B_t; i++) {
        for (int j = 0; j < (S_t/QKVO_BLOCKSIZE); j++) {
          for (int k=0; k < N_t; k++) {
            LIBXSMM_ALIGNED(float tmp[QKVO_BLOCKSIZE * H_t], 64);
            if (b_vnni)
              qgo_brgemm_tpp(&weighted_avg_a[i][j][0][0], &output_w_vnni_a[k][0][0][0], &tmp[0], N_t);
            else
              qgo_brgemm_tpp(&weighted_avg_a[i][j][0][0], &output_w_a[k][0][0][0], &tmp[0], N_t);
            if (bias_flag)
              out_addbias_tpp(&output_b_a[k][0], &tmp[0]);
            out_convert_tpp(&tmp[0], &output_a[i][j][k][0][0]);
          }
