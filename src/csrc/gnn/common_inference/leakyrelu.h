...
RECORD_FUNCTION("leakyrelu", std::vector<c10::IValue>());
...
auto lrelu_tpp = SCOPEIT(LeakyReLUFwdTPP<T>(BS, alpha), ACT);
{
  RECORD_SCOPE(o_lrelu, {t_in});
  {
    RECORD_FUNCTION("parallel_for", std::vector<c10::IValue>());
    long n;
#pragma omp parallel for lastprivate(n)
    for (n = 0; n < ALIGNDOWN(N, BS); n += BS)
      lrelu_tpp(&in[n], &out[n], &lrelu_mask[n / 16]);
    if (n < N) {
      auto lrelu_tpp = SCOPEIT(LeakyReLUFwdTPP<T>(N - n, alpha), ACT);
      lrelu_tpp(&in[n], &out[n], &lrelu_mask[n / 16]);
    }
  }
}
