...
RECORD_FUNCTION("relu", std::vector<c10::IValue>());
...
auto relu_tpp = SCOPEIT(ReLUFwdTPP<T>(BS, true), ACT);
{
  RECORD_SCOPE(o_relu, {t_in});
  {
    RECORD_FUNCTION("parallel_for", std::vector<c10::IValue>());
    long n;
#pragma omp parallel for lastprivate(n)
    for (n = 0; n < ALIGNDOWN(N, BS); n += BS)
      relu_tpp(&in[n], &out[n], &relu_mask[n / 16]);
    if (n < N) {
      auto relu_tpp = SCOPEIT(ReLUFwdTPP<T>(N - n, true), ACT);
      relu_tpp(&in[n], &out[n], &relu_mask[n / 16]);
    }
  }
}
