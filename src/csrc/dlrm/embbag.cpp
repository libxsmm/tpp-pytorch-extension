...
void tpp_embedding_bag_forward_tmpl(torch::Tensor t_weight, torch::Tensor t_input, torch::Tensor t_offsets, torch::Tensor& t_output)
{
  ...
  auto embbag = SCOPEIT((EmbBagFwdTPP<scalar_t, out_scalar_t, int64_t>(E)), ROW_GT);
...
at::Tensor tpp_embedding_bag_forward(torch::Tensor weight, torch::Tensor input, torch::Tensor offsets)
{
  GlobalPass _gp(FWD);
...
template<typename scalar_t>
inline void tpp_embedding_bag_backward_tmpl(torch::Tensor t_gradout, torch::Tensor t_weight, torch::Tensor t_input, torch::Tensor t_offsets, torch::Tensor t_values)
{
  auto N = t_offsets.size(0);
  auto NS = t_input.size(0);
...
at::Tensor tpp_embedding_bag_backward(torch::Tensor gradout, torch::Tensor weight, torch::Tensor input, torch::Tensor offsets)
{
  GlobalPass _gp(BWD);
  auto NS = input.size(0);
  ...
  if (weight.dtype() == at::kFloat) {
    tpp_embedding_bag_backward_tmpl<float>(gradout, weight, input, offsets, values);
  } else if (weight.dtype() == at::kBFloat16) {
    tpp_embedding_bag_backward_tmpl<bfloat16>(gradout, weight, input, offsets, values);
  } else {
    TPP_ASSERT(0, "This datatype is not supported\n");
  }
...
at::Tensor tpp_bdot_forward(at::Tensor &in)
{
  ...
  if (in.scalar_type() == at::kFloat) {
    ...
    auto mm_kernel_f32_tpp = SCOPEIT((BrgemmTPP<float,float>(M, M, K, M*K, M*K, 0.0)));
    auto tr_kernel_tpp = SCOPEIT(XformExtTPP<float>(M, K, XformTPP::XFORM_XPOSE_TPP), XPOSE);
...
#pragma omp parallel for
    ...
    auto mm_kernel_bf16_tpp = SCOPEIT((BrgemmTPP<bfloat16,bfloat16>(M, M, K, M*K, M*K, 0.0)));
    auto tr_kernel_tpp = SCOPEIT(XformExtTPP<at::BFloat16>(M, K, XformTPP::XFORM_XPOSE_N2V_TPP), XPOSE);
...
at::Tensor tpp_bdot_backward(at::Tensor &in1, at::Tensor &in2)
{
  ...
  auto mm_kernel_f32_tpp = SCOPEIT((BrgemmTPP<float,float>(M, N, K, M*K, N*K, 0.0)));
...
REGISTER_SUBMODULE(_embbag_cpp, m) {
  m.def("embbag_forward", &tpp_embedding_bag_forward, "Tpp Embedding Bag forward");
  m.def("embbag_backward", &tpp_embedding_bag_backward, "Tpp Embedding Bag backward");
