  ...
  class SpmmKernel : public BaseTPP {
   public:
    ...
    SpmmKernel(SpmmTPP* p, int config) : p(p), config(config) {
      ...
      auto dt_out = XsmmDtype<Tout>();
      long type = -1;
      unsigned int use_bf16 =
          (XsmmDtype<Tin>() == LIBXSMM_DATATYPE_BF16) ? 1 : 0;
      unsigned int use_i8 = (XsmmDtype<Tin>() == LIBXSMM_DATATYPE_I8) ? 1 : 0;
      unsigned int vnni_block_size = (use_bf16 > 0)
          ? libxsmm_cpuid_dot_pack_factor(LIBXSMM_DATATYPE_BF16)
          : ((use_i8 > 0) ? libxsmm_cpuid_dot_pack_factor(LIBXSMM_DATATYPE_I8)
                          : 1);
      spmm_flags = (use_bf16 == 1 || use_i8 == 1)
          ? (((use_bf16 > 0 && vnni_block_size == 4) ||
              (use_i8 > 0 && vnni_block_size == 8))
                 ? LIBXSMM_GEMM_VNNI_FLAGS('N', 'T', 'V', 'V')
   ...
   protected:
    ...
    void* build_kernel() override {
      ...
      unsigned int vnni_block_size = (use_bf16 > 0)
          ...
          : ((use_i8 > 0) ? libxsmm_cpuid_dot_pack_factor(LIBXSMM_DATATYPE_I8)
                          : 1);
      libxsmm_bitfield l_flags = (use_bf16 == 1 || use_i8 == 1)
          ? (((use_bf16 > 0 && vnni_block_size == 4) ||
              (use_i8 > 0 && vnni_block_size == 8))
                 ? LIBXSMM_GEMM_VNNI_FLAGS('N', 'T', 'V', 'V')
                 : LIBXSMM_GEMM_VNNI_FLAGS('N', 'N', 'V', 'N'))
      ...
      l_shape.comp_type =
          (use_i8 == 0) ? LIBXSMM_DATATYPE_F32 : LIBXSMM_DATATYPE_I32;
      spgemm_config.packed_width = p->M;
      spgemm_config.bk = p->bcsc_bk;
      spgemm_config.bn = p->bcsc_bn;
      l_test_jit.gemm = libxsmm_create_packed_spgemm_bcsc(
          l_shape, l_flags, l_prefetch_flags, spgemm_config);
  ...
  class GemmKernel : public BaseTPP {
   ...
   protected:
    ...
    void* build_kernel() override {
      ...
      l_shape.b_in_type = XsmmDtype<Tin>();
      l_shape.out_type = XsmmDtype<Tout>();
      l_shape.comp_type = (XsmmDtype<Tin>() == LIBXSMM_DATATYPE_F16)
          ? LIBXSMM_DATATYPE_F16
          : LIBXSMM_DATATYPE_F32;

      l_test_jit.gemm =
          libxsmm_dispatch_gemm_v2(l_shape, l_flags, l_prefetch_flags);
