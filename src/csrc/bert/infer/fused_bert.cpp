...
class BertEncoderLayer {
  public:
    ...
    BertEncoderLayer(std::vector<at::Tensor>& params, float eps, long N, long H) : eps(eps), N(N), H(H) {
      int i = 0;
      t_Wq = params[i++];
      t_Wk = params[i++];
      t_Wv = params[i++];
    ...
    inline void qkv_gemm(at::Tensor& t_HS, at::Tensor& t_W, at::Tensor& t_B, at::Tensor& t_Out) {
      ...
      auto xform_tpp = SCOPEIT(XformExtTPP<T>(S2, F2, xform, true), transpose ? XPOSE : VNNI);
      auto copy_bias_tpp = SCOPEIT(CpyBiasTPP<T>(S2, F2), BIAS);
      auto qkv_gemm_tpp = SCOPEITGEMM((BrgemmExtTPP<T, T>(
              S2, F2, F2, S2 * F2, F2 * F2, 1.0, XformTPP::XFORM_NONE_TPP, 0, F1)));
    ...
    template<typename T>
    inline void q_gemm(at::Tensor& t_HS, at::Tensor& t_W, at::Tensor& t_B, at::Tensor& t_Out) {
      qkv_gemm<T, false, false>(t_HS, t_W, t_B, t_Out);
    }

    template<typename T>
    inline void k_gemm(at::Tensor& t_HS, at::Tensor& t_W, at::Tensor& t_B, at::Tensor& t_Out) {
      qkv_gemm<T, true, true>(t_HS, t_W, t_B, t_Out);
    }

    template<typename T>
    inline void v_gemm(at::Tensor& t_HS, at::Tensor& t_W, at::Tensor& t_B, at::Tensor& t_Out) {
      qkv_gemm<T, false, true>(t_HS, t_W, t_B, t_Out);
    }

    template<typename T>
    inline void self_attn(at::Tensor& t_QL, at::Tensor& t_KL_TV, std::vector<at::Tensor>& t_masks, at::Tensor& t_VL_V, at::Tensor& t_CL) {
      auto& t_AM = t_masks[0];
      ...
      auto a_gemm_tpp = SCOPEITGEMM((BrgemmExtTPP<T, float>(
              S2, S2, H2, S2 * H2, H2 * S2, 0.0, XformTPP::XFORM_NONE_TPP, 0, H1)));
...
#pragma omp parallel for collapse(2) schedule(static, 1)
    ...
    inline void output(at::Tensor& t_in, at::Tensor& t_in2, at::Tensor& t_wt, at::Tensor& t_bias, at::Tensor& t_gamma, at::Tensor& t_beta, at::Tensor& t_out) {
...
#pragma omp parallel for
    ...
    template<typename T>
    inline void intermediate(at::Tensor& t_in, at::Tensor& t_wt, at::Tensor& t_bias, at::Tensor& t_out) {
      auto in_sizes = t_in.sizes();
      auto wt_sizes = t_wt.sizes();
      auto S1 = in_sizes[0];
      auto Nc = in_sizes[1];
    ...
    template<typename T, typename LT=T>
    at::Tensor& forward(at::Tensor& t_HS, std::vector<at::Tensor>& t_masks, at::Tensor& t_Out, std::vector<at::Tensor>& t_scratch) {
...
class BertEncoder {
  public:
    ...
    BertEncoder(std::vector<std::vector<at::Tensor>> params, float eps, long N, long HS, long IS) : eps(eps), N(N), HS(HS), IS(IS) {
      H = HS / N;
...
REGISTER_SUBMODULE(_fused_bert_infer, m) {
  py::class_<BertEncoder>(m, "BertEncoder")
    .def(py::init<std::vector<std::vector<at::Tensor>>, float, long, long, long>())
