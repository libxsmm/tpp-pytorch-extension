...
class Qwen3DecoderLayer(BlockedModule):
    def forward(
        self,
...
def FixQwen3DecoderLayer(
    ...
    if not isinstance(self, transformers.models.qwen3.modeling_qwen3.Qwen3DecoderLayer):
        return
    self.__class__ = Qwen3DecoderLayer
    ...
    if not hasattr(self, "cpp_block"):
        # Qwen3 has no qkv bias; the C++ block treats an empty tensor as "no bias"
        no_bias = torch.Tensor().to(layer_dtype)

        def bias_or_empty(m):
            return m.bias if m.bias is not None else no_bias
        ...
        params += [
            bias_or_empty(self.self_attn.q_proj),
            bias_or_empty(self.self_attn.k_proj),
            bias_or_empty(self.self_attn.v_proj),
        ]
        params += [self.self_attn.q_norm.weight, self.self_attn.k_norm.weight]

        params += [embed_positions]

        self.cpp_block = torch.classes.tpp_llm.Qwen3DecoderLayer(
            params,
...
def OptimizeModelForQwen3(model, dtype, device="cpu", weight_dtype=None):
    set_pg()
    ...
    if hasattr(model.config, "use_sliding_window"):
        ...
        ), "use_sliding_window is not supported for Qwen3 yet!"
    if weight_dtype is None:
        weight_dtype = dtype
    embed_positions = None
    for m in model.modules():
        if isinstance(m, transformers.models.qwen3.modeling_qwen3.Qwen3PreTrainedModel):
    ...
    for m in model.modules():
        if isinstance(m, transformers.models.qwen3.modeling_qwen3.Qwen3DecoderLayer):
            FixQwen3DecoderLayer(
                m,
                16,
                64,
                dtype,
...
transformers.models.qwen3.modeling_qwen3.Qwen3Model.forward = Qwen3Model_forward
...
transformers.models.qwen3.modeling_qwen3.Qwen3ForCausalLM.forward = (
    Qwen3ForCausalLM_forward_patched
)
